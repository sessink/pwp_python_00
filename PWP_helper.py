"""
This module contains the helper functions to assist with the running and
analysis of the PWP model.
"""
...
def run_demo2(winds_ON=True, emp_ON=True, heat_ON=True, drag_ON=True):
    ...
    """
    Example script of how to run the PWP model.
    This run uses summertime data from the Atlantic sector of the
    Southern Ocean
    """
    ...
    suffix = 'demo2_1e6diff%s%s%s%s' % (winds_flag, emp_flag, heat_flag,
                                        drag_flag)
    forcing, pwp_out = PWP.run(met_data=forcing_fname,
                               prof_data=prof_fname,
                               suffix=suffix,
...
def set_params(lat, dt=3., dz=1., max_depth=100., mld_thresh=1e-4, dt_save=1.,
               rb=0.65, rg=0.25, rkz=0., beta1=0.6, beta2=20.0, heat_ON=True,
               winds_ON=True, emp_ON=True, drag_ON=True):
...
def prep_data(met_dset, prof_dset, params):
    ...
    if params['emp_ON'] is False:
        print("WARNING: E-P is turned OFF.")
        forcing['emp'][:] = 0.0
        forcing['precip'][:] = 0.0
        forcing['evap'][:] = 0.0

    if params['heat_ON'] is False:
